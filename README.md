...
✔ **디렉토리 구조**
   ...
   |          |-- 현대 : 현대 인물과 관련된 데이터 셋
   |
   |-- 2. 크롤링.ipynb : 데이터 셋을 크롤링하는 코드 파일
   |    
   |-- 3. mkdb.ipynb : 전반적인 코드 파일    
   |    
   |-- 4. chat.py : 실행 파일
   |
   |-- 5. 사진 : readme파일을 작성할 때 필요한 사진 자료
   |
   |-- 6. readme.md : readme 파일
   ```
...
#### 1.2 데이터 전처리
  - **작업**
    ...
    - 텍스트 분할
      ...
      ```
  - **오류 발생 및 해결**
    > 데이터를 embedding하는 과정에서 모델이 요청하는 토큰 수가 text-embedding-3-large의 최대 토큰 수인 1000,000을 넘어 오류 발생
    >
